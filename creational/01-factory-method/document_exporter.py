...
class RegistryExporterCreator:
    ...
    def register_exporter(self, format_name: str, creator_cls: Type[ExporterCreator]) -> None:
    ...
    def create_exporter(self, format_name: str) -> ExporterCreator:
        ...
        if not creator_cls:
            ...
            raise ValueError(f"Exporter for format '{format_name}' is not registered. Available formats: {available}")
        return creator_cls()
...
# Application code
def render_document(registry: RegistryExporterCreator, format_name: str, text: str) -> None:
