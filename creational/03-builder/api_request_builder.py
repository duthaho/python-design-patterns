...
class TimeoutConfig:
    ...
    def validate(self) -> List[str]:
        ...
        if self.total_timeout_seconds < (self.connect_timeout_seconds + self.read_timeout_seconds):
            errors.append("total_timeout_seconds must be >= connect_timeout_seconds + read_timeout_seconds")
        return errors
...
class HTTPRequest:
    ...
    def validate_complete_request(self) -> List[str]:
        ...
        if self.method in {HTTPMethod.GET, HTTPMethod.DELETE, HTTPMethod.HEAD} and self.body is not None:
        ...
        if self.files and self.method not in {HTTPMethod.POST, HTTPMethod.PUT, HTTPMethod.PATCH}:
            errors.append(f"File uploads are only supported with POST, PUT, or PATCH methods")
        return errors
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "authentication": self.authentication.auth_type.value if self.authentication else "none",
            "retry_policy": {
...
class LoggingInterceptor(RequestInterceptor):
    ...
    def intercept(self, request: HTTPRequest) -> HTTPRequest:
        ...
        if request.headers:
            masked_headers = {k: (v if k.lower() not in {"authorization", "api_key"} else "****") for k, v in request.headers.items()}
...
class MethodBodyValidationRule(RequestValidationRule):
    ...
    def validate(self, builder: 'HTTPRequestBuilder') -> List[str]:
        ...
        errors = []
        if builder._method in {HTTPMethod.GET, HTTPMethod.DELETE, HTTPMethod.HEAD} and builder._body is not None:
            errors.append(f"{builder._method.value} requests should not have a body")
        if builder._method in {HTTPMethod.POST, HTTPMethod.PUT, HTTPMethod.PATCH} and builder._body is None and not builder._files:
...
class ContentTypeValidationRule(RequestValidationRule):
    ...
    def validate(self, builder: 'HTTPRequestBuilder') -> List[str]:
        ...
        if builder._body is not None:
            ...
            if isinstance(builder._body, dict) and content_type != ContentType.JSON.value:
                errors.append("JSON body requires Content-Type to be application/json")
            if builder._files and content_type != ContentType.MULTIPART.value:
                errors.append("File uploads require Content-Type to be multipart/form-data")
        return errors if builder._body is not None else []
    
    def applies_to_state(self, state: BuilderState) -> bool:
        # TODO: Return True when headers and body are configured
        return state in {BuilderState.HEADERS_CONFIGURED, BuilderState.BODY_CONFIGURED, BuilderState.READY_TO_BUILD}
...
class HTTPRequestBuilder:
    ...
    def _validate_state_transition(self, from_state: BuilderState, to_state: BuilderState) -> bool:
        ...
        valid_transitions = {
            ...
            BuilderState.METHOD_SET: [BuilderState.HEADERS_CONFIGURED, BuilderState.BODY_CONFIGURED, BuilderState.AUTH_CONFIGURED, BuilderState.POLICIES_CONFIGURED, BuilderState.READY_TO_BUILD],
            BuilderState.HEADERS_CONFIGURED: [BuilderState.BODY_CONFIGURED, BuilderState.AUTH_CONFIGURED, BuilderState.POLICIES_CONFIGURED, BuilderState.READY_TO_BUILD],
            BuilderState.BODY_CONFIGURED: [BuilderState.AUTH_CONFIGURED, BuilderState.POLICIES_CONFIGURED, BuilderState.READY_TO_BUILD],
            BuilderState.AUTH_CONFIGURED: [BuilderState.POLICIES_CONFIGURED, BuilderState.READY_TO_BUILD],
    ...
    def header(self, name: str, value: str) -> 'HTTPRequestBuilder':
        ...
        if self._state not in {BuilderState.METHOD_SET, BuilderState.HEADERS_CONFIGURED, BuilderState.BODY_CONFIGURED, BuilderState.AUTH_CONFIGURED, BuilderState.POLICIES_CONFIGURED}:
    ...
    def query_param(self, name: str, value: str) -> 'HTTPRequestBuilder':
        ...
        if self._state not in {BuilderState.METHOD_SET, BuilderState.HEADERS_CONFIGURED, BuilderState.BODY_CONFIGURED, BuilderState.AUTH_CONFIGURED, BuilderState.POLICIES_CONFIGURED}:
            raise ValueError("Query parameters can only be set after method is configured")
        if not name or not value:
    ...
    def body_raw(self, body: Union[str, bytes]) -> 'HTTPRequestBuilder':
        ...
        if self._state not in {BuilderState.METHOD_SET, BuilderState.HEADERS_CONFIGURED}:
    ...
    def body_form(self, data: Dict[str, str]) -> 'HTTPRequestBuilder':
        ...
        if self._state not in {BuilderState.METHOD_SET, BuilderState.HEADERS_CONFIGURED, BuilderState.BODY_CONFIGURED}:
            raise ValueError("Body can only be set after method is configured")
        if not isinstance(data, dict):
    ...
    def file(self, field_name: str, filename: str, content: Union[bytes, BinaryIO], 
             content_type: str = None) -> 'HTTPRequestBuilder':
        """Add file upload"""
        # TODO: Implement
        ...
        file_upload = FileUpload(field_name=field_name, filename=filename, content=content, content_type=content_type or "application/octet-stream")
    ...
    def auth_basic(self, username: str, password: str) -> 'HTTPRequestBuilder':
        ...
        if self._state not in {BuilderState.METHOD_SET, BuilderState.HEADERS_CONFIGURED, BuilderState.BODY_CONFIGURED}:
            raise ValueError("Authentication can only be set after method is configured")
        if not username or not password:
            raise ValueError("Username and password cannot be empty")
        self._authentication = Authentication(auth_type=AuthType.BASIC, username=username, password=password)
    ...
    def auth_api_key(self, api_key: str, header_name: str = "X-API-Key") -> 'HTTPRequestBuilder':
        ...
        self._authentication = Authentication(auth_type=AuthType.API_KEY, api_key=api_key, custom_headers={header_name: api_key})
        self._transition_to_state(BuilderState.AUTH_CONFIGURED)
        return self
    ...
    def auth_custom(self, headers: Dict[str, str]) -> 'HTTPRequestBuilder':
        ...
        self._authentication = Authentication(auth_type=AuthType.CUSTOM, custom_headers=headers)
        self._transition_to_state(BuilderState.AUTH_CONFIGURED)
        return self
    ...
    def retry(self, strategy: RetryStrategy, max_attempts: int = 3, 
              base_delay: float = 1.0) -> 'HTTPRequestBuilder':
        """Configure retry policy"""
        # TODO: Implement
        # - Create RetryPolicy object
        # - Validate configuration
        # - Transition to POLICIES_CONFIGURED state
        if self._state not in {BuilderState.METHOD_SET, BuilderState.HEADERS_CONFIGURED, BuilderState.BODY_CONFIGURED, BuilderState.AUTH_CONFIGURED}:
        ...
        self._retry_policy = RetryPolicy(strategy=strategy, max_attempts=max_attempts, base_delay_seconds=base_delay)
        self._transition_to_state(BuilderState.POLICIES_CONFIGURED)
        return self
    
    def timeout(self, connect: float = 30.0, read: float = 30.0, 
                total: float = 60.0) -> 'HTTPRequestBuilder':
        """Configure timeout settings"""
        # TODO: Implement
        # - Create TimeoutConfig object
        # - Validate configuration
        if self._state not in {BuilderState.METHOD_SET, BuilderState.HEADERS_CONFIGURED, BuilderState.BODY_CONFIGURED, BuilderState.AUTH_CONFIGURED, BuilderState.POLICIES_CONFIGURED}:
            raise ValueError("Timeout configuration can only be set after method is configured")
        if connect <= 0 or read <= 0 or total <= 0:
            raise ValueError("Timeout values must be positive")
        self._timeout_config = TimeoutConfig(connect_timeout_seconds=connect, read_timeout_seconds=read, total_timeout_seconds=total)
        self._transition_to_state(BuilderState.POLICIES_CONFIGURED)
        return self
    
    def cache(self, strategy: CacheStrategy, ttl_seconds: int = 300) -> 'HTTPRequestBuilder':
        """Configure caching"""
        # TODO: Implement
        if self._state not in {BuilderState.METHOD_SET, BuilderState.HEADERS_CONFIGURED, BuilderState.BODY_CONFIGURED, BuilderState.AUTH_CONFIGURED}:
            raise ValueError("Cache configuration can only be set after method is configured")
    ...
    def circuit_breaker(self, failure_threshold: int = 5, 
                       timeout_seconds: float = 60.0) -> 'HTTPRequestBuilder':
        """Configure circuit breaker"""
        # TODO: Implement
        if self._state not in {BuilderState.METHOD_SET, BuilderState.HEADERS_CONFIGURED, BuilderState.BODY_CONFIGURED, BuilderState.AUTH_CONFIGURED}:
            raise ValueError("Circuit breaker configuration can only be set after method is configured")
        if failure_threshold < 1:
            raise ValueError("failure_threshold must be at least 1")
        if timeout_seconds <= 0:
            raise ValueError("timeout_seconds must be positive")
        self._circuit_breaker = CircuitBreakerConfig(enabled=True, failure_threshold=failure_threshold, timeout_seconds=timeout_seconds)
        self._transition_to_state(BuilderState.POLICIES_CONFIGURED)
        return self
    ...
    def add_request_interceptor(self, interceptor: RequestInterceptor) -> 'HTTPRequestBuilder':
    ...
    def add_response_interceptor(self, interceptor: ResponseInterceptor) -> 'HTTPRequestBuilder':
    ...
    def with_rate_limiting(self, requests_per_second: float) -> 'HTTPRequestBuilder':
        ...
        self.add_request_interceptor(RateLimitInterceptor(requests_per_second=requests_per_second))
        return self
    
    def with_request_signing(self, secret_key: str, algorithm: str = "HMAC-SHA256") -> 'HTTPRequestBuilder':
        """Enable request signing"""
        # TODO: Implement using RequestSigningInterceptor
        self.add_request_interceptor(RequestSigningInterceptor(secret_key=secret_key, algorithm=algorithm))
        return self
    ...
    def build_and_execute(self) -> Any:
        ...
        while True:
            try:
                ...
                attempts += 1
                if self._retry_policy.strategy == RetryStrategy.NONE or attempts >= self._retry_policy.max_attempts:
                    logger.error(f"Request failed after {attempts} attempts: {e}")
                    raise
                delay = self._retry_policy.get_delay(attempts)
                logger.warning(f"Request failed (attempt {attempts}), retrying in {delay:.2f} seconds: {e}")
                time.sleep(delay)
    ...
    def to_curl(self) -> str:
        ...
        if self._body:
            if isinstance(self._body, str):
                ...
                curl_parts.append(f"--data-binary '{self._body.decode('utf-8', errors='ignore')}'")
        if self._url:
...
class HTTPRequestDirector:
    ...
    def json_api_request(self, method: HTTPMethod, url: str, 
                        data: Dict[str, Any] = None, 
                        api_key: str = None) -> HTTPRequest:
        ...
        return self._builder.reset()\
            ...
            .auth_api_key(api_key) if api_key else self._builder\
            .with_logging()\
            .retry(RetryStrategy.EXPONENTIAL, max_attempts=3)\
            .build()
    
    def file_upload_request(self, url: str, files: List[FileUpload], 
                           form_data: Dict[str, str] = None) -> HTTPRequest:
    ...
    def secure_api_request(self, method: HTTPMethod, url: str, 
                          secret_key: str, data: Dict[str, Any] = None) -> HTTPRequest:
    ...
    def microservice_request(self, service_name: str, endpoint: str, 
                           method: HTTPMethod = HTTPMethod.GET,
                           correlation_id: str = None) -> HTTPRequest:
